...
def run_experiment():
    ...
    # 5. Export Raw Telemetry to CSV for Kaggle
    import csv
    csv_file = "narma10_telemetry.csv"
    print(f"[SAVE] Exporting raw telemetry to {csv_file}...")
    with open(csv_file, mode='w', newline='') as f:
        writer = csv.writer(f)
        # Header: u, y_target, and state features
        header = ['step', 'u_input', 'y_target']
        for i in range(SHARES_PER_STEP - 1): header.append(f'jitter_{i}')
        for i in range(SHARES_PER_STEP): header.append(f'nonce_{i}')
        writer.writerow(header)
        
        for t in range(total_steps):
            row = [t, u[t], y[t]] + X[t].tolist()
            writer.writerow(row)
    print(f"[OK] Telemetry export complete.")
