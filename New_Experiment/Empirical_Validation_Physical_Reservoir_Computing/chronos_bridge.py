...
try:
    ...
    def entropy(pk):
        pk = np.array(pk, dtype=float)  # Ensure float type
...
class ChronosBridge:
    def __init__(self):
        ...
        # ENHANCED: Buffer for individual share timestamps (for Reservoir Computing experiment)
        # CRITICAL: Large buffer to handle infrequent polling (Time Anchor pattern)
        # This prevents WiFi saturation - bridge accumulates locally, experiment fetches in batches
        self.share_timestamps = []  # Store timestamps in seconds (converted from ns)
        self.timestamps_lock = threading.Lock()
        self.max_timestamps = 100000  # ENHANCED: Increased buffer for extended experiments
        self.share_count = 0  # Track total shares processed
        self.last_batch_size = 0  # Track last batch size for monitoring
        print(f"⏳ CHRONOS LISTENER OPENED on {HOST_IP}:{PORT}")

        # Start Threads
        threading.Thread(target=self.api_server, daemon=True).start()
        threading.Thread(target=self.telemetry_loop, daemon=True).start()
        threading.Thread(target=self.wifi_monitor_loop, daemon=True).start()  # ENHANCED: WiFi bottleneck monitoring
    ...
    def wifi_monitor_loop(self):
        """ENHANCED: Monitor WiFi bottleneck and buffer usage"""
        print("📡 WIFI MONITOR STARTED - Time Anchor pattern active")
        while True:
            time.sleep(10)  # Check every 10 seconds
            
            with self.timestamps_lock:
                buffer_usage = len(self.share_timestamps)
                usage_percent = (buffer_usage / self.max_timestamps) * 100
            
            # Alert if buffer is getting full (potential WiFi bottleneck)
            if usage_percent > 80:
                print(f"⚠️  WIFI ALERT: Buffer {usage_percent:.1f}% full ({buffer_usage}/{self.max_timestamps})")
                print(f"   💡 Consider increasing poll_interval in experiment to reduce WiFi traffic")
            elif buffer_usage > 0 and self.last_batch_size == 0:
                print(f"📊 Buffer Status: {buffer_usage} timestamps accumulated (Time Anchor working)")
    ...
    def api_server(self):
        ...
        while True:
            try:
                ...
                elif data == "GET_SHARE_TIMESTAMPS":
                    # ENHANCED: Get individual share timestamps for Reservoir Computing experiment
                    # Returns ALL accumulated timestamps since last fetch (batch mode)
                    # This supports "Time Anchor" pattern: infrequent polling prevents WiFi saturation
                    with self.timestamps_lock:
                        # Return timestamps and clear buffer (packet mode for batch fetch)
                        timestamps_copy = list(self.share_timestamps)
                        self.share_timestamps = []
                        batch_size = len(timestamps_copy)
                        self.last_batch_size = batch_size
                    
                    # Send as JSON array (single transmission minimizes WiFi traffic)
                    response_data = json.dumps(timestamps_copy)
                    conn.sendall(response_data.encode())
                    
                    # ENHANCED: WiFi-friendly logging
                    if batch_size > 0:
                        print(f"📤 Batch sent: {batch_size} timestamps (Total: {self.share_count})")
                    # Note: Single send is better for WiFi than multiple small packets
    ...
    def process_message(self, msg, conn, timestamp):
        ...
        if method == 'mining.subscribe':
            ...
            # Convert timestamp from nanoseconds to seconds for Reservoir Computing experiment
            timestamp_sec = timestamp / 1e9
            
            # Store individual timestamp in buffer (for Reservoir Computing)
            with self.timestamps_lock:
                self.share_timestamps.append(timestamp_sec)
                self.share_count += 1
                # Keep buffer size manageable
                if len(self.share_timestamps) > self.max_timestamps:
                    self.share_timestamps = self.share_timestamps[-self.max_timestamps:]

            # GUARDAR SOLO EL TIEMPO (for rhythm analysis - keep in ns)
