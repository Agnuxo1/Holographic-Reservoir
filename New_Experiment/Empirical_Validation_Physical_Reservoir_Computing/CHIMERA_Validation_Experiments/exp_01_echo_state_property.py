...
CONFIG = {
    "miner_ip": "192.168.1.100",  # <-- CHANGE THIS to your miner's IP
    "miner_port": 80,
    "api_timeout": 10,
...
class MinerInterface:
    ...
    def set_voltage(self, voltage_mv: int) -> bool:
        ...
        try:
            url = f"{self.base_url}/api/system"
            payload = json.dumps({"voltage": voltage_mv}).encode()
            req = urllib.request.Request(url, data=payload, method='PATCH')
            req.add_header('Content-Type', 'application/json')
            with urllib.request.urlopen(req, timeout=self.timeout) as response:
                return response.status == 200
...
class ESPExperiment:
    ...
    def collect_timing_data(self, duration_sec: float, 
                           poll_interval: float = 0.5) -> List[float]:
        """Collect share timestamps for a duration"""
        ...
        return self.miner.get_and_clear_shares()
    ...
    def run_single_trial(self, trial_type: str, trial_config: Dict, 
        ...
        prehistory_timestamps = self.collect_timing_data(
            self.config["prehistory_duration_sec"]
        )
        
        telemetry = self.miner.get_telemetry()
        prehistory_stats = compute_timing_statistics(prehistory_timestamps)
        
        prehistory_measurement = TimingMeasurement(
            trial_type=trial_type,
            repetition=repetition,
            phase="prehistory",
            share_timestamps=prehistory_timestamps,
            num_shares=len(prehistory_timestamps),
            mean_interarrival_ms=prehistory_stats["mean_ms"],
            std_interarrival_ms=prehistory_stats["std_ms"],
            cv=prehistory_stats["cv"],
            histogram_entropy=prehistory_stats["entropy"],
        ...
        self.log(f"[Prehistory] Shares: {len(prehistory_timestamps)}, "
                f"CV: {prehistory_stats['cv']:.3f}")
        ...
        test_timestamps = self.collect_timing_data(
            self.config["measurement_window_sec"]
        )
        
        telemetry = self.miner.get_telemetry()
        test_stats = compute_timing_statistics(test_timestamps)
        
        test_measurement = TimingMeasurement(
            trial_type=trial_type,
            repetition=repetition,
            phase="test",
            share_timestamps=test_timestamps,
            num_shares=len(test_timestamps),
            mean_interarrival_ms=test_stats["mean_ms"],
            std_interarrival_ms=test_stats["std_ms"],
            cv=test_stats["cv"],
            histogram_entropy=test_stats["entropy"],
            frequency_mhz=telemetry.get("frequency_mhz", 0),
            voltage_mv=telemetry.get("voltage_mv", 0),
            temperature_c=telemetry.get("temperature_c", 0),
        ...
        self.log(f"[Test] Shares: {len(test_timestamps)}, "
                f"CV: {test_stats['cv']:.3f}, Entropy: {test_stats['entropy']:.2f}")
