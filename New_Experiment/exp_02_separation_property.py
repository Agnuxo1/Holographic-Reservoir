...
CONFIG = {
    ...
    # Timing parameters
    "step_duration_sec": 60,         # Increased for bridge stabilization
    "measurement_window_sec": 45,    # Window for collecting statistics
    "stabilization_delay_sec": 45,   # Wait after restart
...
class MinerInterface:
    ...
    def set_frequency(self, freq_mhz: int) -> bool:
        """Set mining frequency - CRITICAL: Requires miner restart to apply"""
        try:
...
class SeparationExperiment:
    ...
    def collect_state(self, duration_sec: float, 
        ...
        state = StateVector(
            ...
            n_shares=int(m.get("sps", 0) * duration_sec)
        )
        return state
    ...
    def run_pattern(self, pattern_name: str, pattern_config: Dict,
        ...
        for step_idx, offset in enumerate(sequence):
            ...
            # Set frequency
            self.bridge.inject_seed(f"SEPARATION_{pattern_name}_{repetition}_step{step_idx}")
...
if __name__ == "__main__":
    print("=" * 70)
    print("CHIMERA VALIDATION EXPERIMENT 2")
    print("Separation Property Test")
    print()
    print("This experiment tests whether the ASIC can distinguish")
    print("between different input patterns - a key reservoir property.")
    print()
    print("Expected duration: TIME STOPPED - We prioritize REAL data")
    print("=" * 70)
    print()
